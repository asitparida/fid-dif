...
export class AuthoringComponent implements OnInit {
    ...
    constructor() { }
    ngOnInit() {
        this.fidelityCollection.forEach((x: any, i: number) => {
            x.stateName = BunBunStates[i].state;
            x.leftImgSrc = `url('${x.leftImg}')`;
            x.rightImgSrc = `url('${x.rightImg}')`;
            x.leftMarkers = BunBunStates[i].leftPage.markers;
            x.rightMarkers = BunBunStates[i].rightPage.markers;
        });
    }
